...
固定历史研究使用 `/usr/local/xqsim/data/futures/cc_2024`；该快照由
`tools/futures/snapshot_cache.py --cutoff 20241231` 从完整缓存生成，所有矩阵和逐日
压缩数据以及 `DateIndex.csv` 均截止 2024-12-31，InstrumentIndex 不保留 2025 年后
才开始的合约行。
...
- **EndDate 排他语义（重要）**：加载器对每行按 `[StartDate, EndDate)` 左闭右开填充
  （EndDate 当天无效）。期货 meta 生成器将 wind 最后交易日转换成其**下一交易日**
  作为 EndDate；仍在市合约使用远期哨兵。StartDate 和落在日历范围内的 EndDate
  必须是日历中的交易日；EndDate 晚于有限历史日历时，合约有效期包含日历最后一天。
  同一槽位的多段**只需不重叠**，段间空窗是正常状态
