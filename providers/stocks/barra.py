...
from xqsim.common_utils import *
...
class Provider(StaticProvider):
    ...
    def generate_dly_factor_return(self):
        sql = """
            ...
            """ % (self.meta.begin_trading_day, self.meta.end_trading_day)
        array = np.full((self.meta.di_size, len(self.factor_dict)), np.nan, np.float64)
        for row in self.exec_sql_fetchall(sql):
    ...
    def generate_exposure(self):
        ...
        for di in range(self.meta.begin_di, self.meta.end_di + 1, batch_size):
            ...
            exposure_array = np.full((end_di - start_di + 1, self.meta.ii_size, len(self.factor_dict)), np.nan, np.float64)
    ...
    def generate_covariance(self, dbname, data_name):
        ...
        covariance_array = np.full((self.meta.di_size, len(self.factor_dict), len(self.factor_dict)), np.nan, np.float64)
        for row in self.exec_sql_fetchall(sql):
            trading_day = int(row["TradingDay"])
            di = self.meta.offset_di_mapping[trading_day]
...
def main():
    builder_run(meta_dir=STOCKS_CC_DIR, begin_date=20201208, end_date=20201231, output_cache_dir=STOCKS_UPDATE_DIR)
