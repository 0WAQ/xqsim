...
class MetaLoader(object):
    ...
    def __load_static_size(self, meta_dir: str):
        ...
        if self.__meta.ii_size == 0:
            abort("Load ii size failed")
        log_info("Load ii size %s", self.__meta.ii_size)
    ...
    def __load_date_index(self, meta_dir: str):
        ...
        for i in range(len(lines)):
            ...
            self.__meta.total_di_mapping[trading_day] = di
    ...
    def __get_di(self, date: str, cmp_func):
        date_str = str(date)
        if date_str.startswith("TODAY"):
            diff_day = 0
            now = datetime.datetime.now()
            today_date = int("%04d%02d%02d" % (now.year, now.month, now.day))
    ...
    def __check_date(self, config_dict):
        ...
        self.__load_instrument_index(self.__meta.meta_dir, begin_di - back_days, end_di, self.__config_dict.get("simple_index", False))
        ...
        if config_dict.get("checkpoint") == "true":
            ...
            check_date_path = os.path.join(self.__meta.checkpoint_dir, ".date")
    ...
    def __load_instrument_index(self, meta_dir: str, load_begin_di, load_end_di, simple_index=False):
