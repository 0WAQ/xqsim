# 对目标日 di 的 v1 做 signed-power × 严格滞后逆波动映射。
# InvVol60 使用收益窗口 [di-60, di)，不读取目标日 di 的收益。
import numpy as np
...
class Operation(OperationBase):
    def __init__(self, *args):
        super().__init__(*args)
        self.power = float(simcfg.get(self.cfg, "power", 2.0))                      # type: ignore
        self.window = int(simcfg.get(self.cfg, "inverse_vol_window", 60))           # type: ignore
        self.floor_quantile = float(
            simcfg.get(self.cfg, "inverse_vol_floor_quantile", 0.10)
        )                                                                           # type: ignore
        self.min_names = int(simcfg.get(self.cfg, "min_names", 15))                 # type: ignore
        ...
        log_info(
            "AlphaJrxDaily PowerInvVol: power %.3f, window %d, products %d",
            self.power,
            self.window,
            self.product_count,
        )
