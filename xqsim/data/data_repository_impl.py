...
from xqsim.data.data_repository import *
...
class DataHolder(object):
    def __init__(self, array: np.ndarray, offset_di: int, name: str, dr: DataRepository, data_info: DataInfo, header: DataHeader):
        ...
        self.next_reload_di: int | None = None
        self.pre_load_di: int | None = None
        self.back_days: int = 0
        self.last_ti_list: list = [None, -1]
...
class DataRepositoryImpl(DataRepository):
    ...
    def __load_segment_data(self, name: str, header: DataHeader, data_info: DataInfo) -> DataHolder:
    ...
    def __load_data(self, data_info: DataInfo, header: DataHeader, begin_di, end_di,
                    new_header_list=None, begin_ti: int | None = None, end_ti: int | None = None, only_refresh=False) -> np.ndarray:
        data = None
        shm_path = None
    ...
    def write_data(self, dir_name: str, data_name: str, data: np.ndarray, 
        ...
        self.__data_manager.write_data(output_dir, data_name, data, begin_trading_day, 
                                       end_trading_day, data_type, dimension, dtype, overwrite, compress)

    def write_compress_data(self, dir_name: str, data_name: str, data: np.ndarray, begin_trading_day: int | None = None, end_trading_day: int | None = None,
                            data_type: str = DataManager.TYPE_DATA, dimension: str = "", dtype: np.dtype | None = None, overwrite=False):

        output_dir = self.__get_output_dir(dir_name, data_type)
        self.__data_manager.write_compress_data(output_dir, data_name, data, begin_trading_day, 
                                                end_trading_day, data_type, dimension, dtype, overwrite)
    ...
    def append_data(self, dir_name: str, data_name: str, data: np.ndarray, 
        ...
        output_dir = self.__get_output_dir(dir_name, data_type)
        self.__data_manager.append_data(output_dir, data_name, data, begin_trading_day, 
                                        end_trading_day, data_type, dimension, dtype, part_overwrite)
