...
def load_cls(file_path, cls_name):
...
class AlphaTask(object):
    ...
    def add_alpha(self, alpha_id, file_path, temp_config=None):
        ...
        cls = load_cls(config["file_path"], "Alpha")
    ...
    def add_alpha_cls(self, cls, temp_config=None):
        config = self.__local_config.copy()
        if temp_config:
            config.update(temp_config)
    ...
    def add_module(self, temp_config: dict, cls_type, cls_list):
        config = self.__local_config.copy()
        if temp_config:
            config.update(temp_config)
        config["file_path"] = os.path.abspath(config["file_path"])
    ...
    def calculate_stats(self, di):
        if len(self.stats_list) == 0:
            return
        value = None
        for stats in self.stats_list:
            value = stats.calculate_di(di, self.__alpha.alpha)
        return value

    def run_before(self, di):
    ...
    def run_portfolio(self, di, alpha_list):
        univbase.instruments = self.__meta.instrument_index[di]
        self.__save_csv_list.append([di - self.__alpha.delay, None, None, None])
...
class AlphaManager(object):
    def __init__(self, dr: DataRepository):
        self.__dr = dr
        self.__meta = dr.meta
        self.__alpha_task_list = []
        self.__portfolio_task = None
        self.__all_task = []
    ...
    def create_alpha_task(self, config=None) -> AlphaTask:
    ...
    def create_portfolio_task(self, config=None) -> AlphaTask:
        local_config = self.__meta.global_cfg.copy()
        if config is not None:
            local_config.update(config)
