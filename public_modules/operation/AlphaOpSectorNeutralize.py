"""Cross-sectionally rank, normalize, and sector-neutralize alpha values."""
...
class Operation(OperationBase):
    def __init__(self, *args):
        super().__init__(*args)
        self.min_cs = int(simcfg.get(self.cfg, "min_cs", 15))
        self.min_unique = int(simcfg.get(self.cfg, "min_unique", 5))
        self.sector = self.dr.get_data("ind.sector")
        log_info(
            "SectorNeutralize: min_cs %d, min_unique %d",
            self.min_cs,
            self.min_unique,
        )
