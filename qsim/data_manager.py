...
class DataManager(object):
    ...
    def get_path(self, dir_path: str, data_name: str, data: np.ndarray, data_type: str, dimension: str = "", dtype: np.dtype = None, trading_day: str = None, shape=None,
                 compress=True):
        if shape is None:
            shape = data.shape
        if dimension == "":
    ...
    def check_date(self, begin_trading_day: int, end_trading_day: int):
    ...
    def write_data(self, dir_path: str, data_name: str, data: np.ndarray, begin_trading_day: int = None, end_trading_day: int = None,
                   data_type: str = TYPE_DATA, dimension: str = "", dtype: np.dtype = None, overwrite=False, compress=False):
    ...
    def write_compress_data(self, dir_path: str, data_name: str, data: np.ndarray, begin_trading_day: int = None, end_trading_day: int = None,
                            data_type: str = TYPE_DATA, dimension: str = "", dtype: np.dtype = None, overwrite=False):
        if begin_trading_day is None:
            begin_trading_day = self.__meta.begin_trading_day
        if end_trading_day is None:
            end_trading_day = self.__meta.end_trading_day
    ...
    def append_data(self, dir_path: str, data_name: str, data: np.ndarray, begin_trading_day: int = None, end_trading_day: int = None,
                    data_type: str = TYPE_DATA, dimension: str = "", dtype: np.dtype = None, part_overwrite=False):
        file_path = self.get_path(dir_path, data_name, data, data_type, dimension, dtype)
    ...
    def append_data_by_ti(self, dir_path: str, data_name: str, data: np.ndarray, trading_day: int, ti: int, ti_size: int, data_type: str = TYPE_DATA, date_dir=True):
        ...
        if not os.path.exists(file_path):
            ...
            with open(file_path, "wb") as writer:
                ...
                nan_array = np.full((ti_size, ii_size), NAN, np.float64)
                nan_array[ti] = data
            ...
            with open(file_path, "r+b") as writer:
                ...
                if new_end_di > cur_end_di:
                    # append di data
                    nan_array = np.full((ti_size, data.shape), NAN, np.float64)
                    nan_array[ti] = data
    ...
    def data_exist(self, dir_path: str, data_name: str, data: np.ndarray, data_type: str, dimension: str = "", dtype: np.dtype = None):
        file_path = self.get_path(dir_path, data_name, data, data_type, dimension, dtype)
        return os.path.exists(file_path)

    def remove_data(self, dir_path: str, data_name: str, data: np.ndarray, data_type: str,
                    dimension: str = "", dtype: np.dtype = None):
        file_path = self.get_path(dir_path, data_name, data, data_type, dimension, dtype)
    ...
    def write_data_to_file(self, file_path: str, data: np.ndarray, begin_trading_day: int, end_trading_day: int):
    ...
    def append_data_to_file(self, file_path: str, data: np.ndarray, begin_trading_day: int, end_trading_day: int, part_overwrite: bool):
        begin_trading_day, end_trading_day, di_size = self.check_date(begin_trading_day, end_trading_day)
    ...
    def load_data_from_file(self, file_path: str, begin_trading_day: int = None, end_trading_day: int = None, total=False, out=None, fillna=False,
                            begin_ti: int = None, end_ti: int = None) -> np.ndarray:
    ...
    def load_file_data(self, file_path: str, begin_trading_day: int = None, end_trading_day: int = None, out=None,
                       shm_path=None, fillna=False, header_list=None, begin_ti: int = None, end_ti: int = None) -> np.ndarray:
        data = out
    ...
    @staticmethod
    def get_dir_path(dir_path: str, date: str, file_name: str, shm_path: str) -> (bool, str):
    ...
    def load_dir_data(self, dir_path: str, file_name: str, data: np.ndarray, begin_trading_day: int = None, end_trading_day: int = None, total=False,
                      shm_path=None, fillna=False, header_list=None, begin_ti: int = None, end_ti: int = None) -> np.ndarray:
        header = None
        if total is False:
            begin_di = self.__meta.di_mapping[begin_trading_day]
        ...
        for i in range(len(trading_day_list)):
            ...
            header = self.load_data_with_header(file_path, trading_day_list[i], trading_day_list[i], out=data[i], begin_ti=begin_ti, end_ti=end_ti)[1]
    ...
    @staticmethod
    def load_dir_header(dir_path, date, file_name, shm_path=None) -> DataHeader:
    ...
    def load_data_with_header(self, file_path: str, begin_trading_day: int = None, end_trading_day: int = None, total=False, out=None,
                              fillna=False, begin_ti: int = None, end_ti: int = None) -> (np.ndarray, DataHeader):
        data_header = DataHeader()
        with open(file_path, "rb") as reader:
            data_header.decode(reader.read(DATA_HEADER_LENGTH))
            ...
            if file_path.endswith(DataManager.TYPE_LZ4):
                ...
                if out is None:
                    if forward_fill_di_size + backward_fill_di_size > 0:
                        # fill nan
                        data = np.full(shape, NAN, np.float64).ravel()
